...
//for mobile app view
exports.facebook_signin_and_signup=async (req,res,next)=>{
  //    const {userFirstname, userEmail, userGoogle_Id } = req.body;
     ...
     try {//checks if user exist first and if so, user shall be directed to sign in 
      let user = await User.findOne({facebookId:userFacebook_Id,email:userEmail})//({email:userEmail})//find user by ID // email: userEmail
      if (user){
